...
import { validateEffectiveDate, MAX_BACKDATE_DAYS } from "../effectiveDate";
...
test("today is valid and not backdated", () => {
  const result = validateEffectiveDate(daysAgo(0));
  assert.equal(result.isBackdated, false);
  assert.equal(result.dateStr, todayInOrgTimezone());
});

test("yesterday works and is backdated", () => {
  const result = validateEffectiveDate(daysAgo(1));
  assert.equal(result.isBackdated, true);
  assert.equal(result.dateStr, daysAgo(1));
});

test("exactly 299 days ago works", () => {
  const result = validateEffectiveDate(daysAgo(299));
  assert.equal(result.isBackdated, true);
});

test("exactly 300 days ago (the boundary) is accepted", () => {
  assert.equal(MAX_BACKDATE_DAYS, 300);
  const result = validateEffectiveDate(daysAgo(300));
  assert.equal(result.isBackdated, true);
});

test("301 days ago is rejected", () => {
  assert.throws(() => validateEffectiveDate(daysAgo(301)), /more than 300 days/);
});

test("future date is rejected", () => {
  assert.throws(() => validateEffectiveDate(daysAgo(-1)), /future/);
  assert.throws(() => validateEffectiveDate(daysAgo(-30)), /future/);
});

test("malformed date strings are rejected", () => {
  assert.throws(() => validateEffectiveDate("not-a-date"));
  assert.throws(() => validateEffectiveDate("2026/07/24"));
  assert.throws(() => validateEffectiveDate(""));
  assert.throws(() => validateEffectiveDate(undefined));
  assert.throws(() => validateEffectiveDate(12345));
});

test("referenceDate resolves to noon on the requested date in the org timezone", () => {
  const target = daysAgo(10);
  const result = validateEffectiveDate(target);
