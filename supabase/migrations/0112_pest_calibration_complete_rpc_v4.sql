-- Pest Control device Calibration module — completion RPC (v4).
--
-- Rebuilt after 0111 removed the "Overall Result" concept: no more
-- overall_result_mode_snapshot/calculated_result/recorded_result/
-- result_discrepancy parameters — a completion is just the device/template
-- snapshot plus whatever ordinary task answers were recorded. Those four
-- columns stay on pest_calibration_records (now nullable) purely so
-- existing historical rows keep their original values; new rows simply
-- never populate them (they land as NULL via the column default).
-- Otherwise unchanged: still one plpgsql function body = one atomic
-- transaction, still idempotent via completion_request_id + ON CONFLICT DO
-- NOTHING short-circuit, still service_role-only EXECUTE.
