-- Pest Control device Calibration module — schema (v2).
--
-- Recreated fresh after 0105 dropped the old objects — see 0105's header
-- for why (0103 was already marked "applied" on hosted Supabase under an
-- older version of this same content, and migration tracking is by version
-- number, not content, so simply editing 0103 again would never re-run
-- there). This file's actual table/policy/trigger definitions are
-- otherwise unchanged from 0103's current content.
--
